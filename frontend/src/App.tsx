...
function App() {
  ...
  //useState for sidebar, MyNotes, isNewForm visiblity
  ...
  //useEffect to fetch all Notes once
  useEffect(()=>{
  ...
  //Update checked boxes for action items
  ...
  //Save Edited Notes with backend and sync with Mynotes state
  ...
  return (
    <>
     ...
     { isNewForm &&  <NewNoteForm createNewNote={createNewNote}/>}
      {!isVisibleMynotes && <WelcomeMessage/> }
      { isVisibleMynotes && (
         <ContentSection 
         MyNotes={myNotes} 
